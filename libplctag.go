...
// #//  c g o CFLAGS: -I${SRCDIR}/third_party/libplctag/src/lib -g -Wall
...
func TagStatus(tag TagID) (int, error) {
	res := int(C.plc_tag_status(C.int32_t(tag)))
	if res < 0 {
		return res, errors.New(TagDecodeError(res))
	}
	return res, nil
}
...
func TagSetInt32(tag TagID, offset int, val int32) error {
	res := int(C.plc_tag_set_int32(C.int32_t(tag), C.int(offset), C.int32_t(val)))
	if res < 0 {
		es := TagDecodeError(res)
		return errors.New(es)
	}
